...
def color_dataset(images, labels, environment, label_noise_rate=0.25, subsample=True, int_target=False, cuda=None):
    if subsample:
    ...
    if int_target:
        y = y.long()
    use_cuda = bool(cuda) if cuda is not None else torch.cuda.is_available()
...
def get_cmnist_datasets(root, train_envs=(0.1, 0.2), test_envs=(0.9,), label_noise_rate=0.25,
                        dataset_transform=color_dataset, subsample=True, int_target=False, cuda=None,
                        use_test_set=False, download=True):
    if root is None:
        raise ValueError('Data directory not specified!')

    orig_data_tr = MNIST(root, train=True, download=download)
    perm_inds_tr = torch.randperm(len(orig_data_tr.data))        # permute / shuffle

    if use_test_set:
        # make use of mnist test set to create test envs
        orig_data_tst = MNIST(root, train=False, download=download)
