...
from iro.utility.iro_utils import AggregationFunction as aggregation_function, Pareto_distribution

from .kde import Nonparametric, get_grad_norm
...
class ERM(Algorithm):
    ...
    def __init__(self, network, hparams, loss_fn=None):
        # Backward compatibility with older signature: (network, loss_fn, hparams)
        if callable(hparams) and isinstance(loss_fn, dict):
            hparams, loss_fn = loss_fn, hparams
        if loss_fn is None:
            raise ValueError("loss_fn must be provided.")
        super(ERM, self).__init__(network, hparams, loss_fn)
