...
class AggregationFunction:
    ...
    def __init__(self, name: str, custom_func: callable = None):
        ...
        name : str
            ...
            (options: "cvar", "var", "entropic", "mean",
                      ...
                      "tail_var", "ph", "g_entropic", "wang",
                      "exponential"/"esrm", "evar", "custom")
    ...
    def aggregate(self, risks, alpha: float = 0.05, eta: float = 1.0, weights=None, xi: float = 1.0, **kwargs) -> float:
        ...
        **kwargs : additional keyword arguments
        ...
        elif self.name in ("exponential", "esrm"):
            gamma = kwargs.get("gamma", alpha if alpha is not None else eta)
            return self.__exponential__(risks, gamma)
        elif self.name == "evar":
            return self.__evar__(risks, alpha)
        elif self.name == "mean":
            if weights is None:
    ...
    def __exponential__(self, risks: torch.Tensor, gamma: float) -> torch.Tensor:
        """Exponential spectral risk measure (Arrow-Pratt coefficient gamma > 0)."""
        gamma_t = torch.as_tensor(gamma, dtype=risks.dtype, device=risks.device)
        gamma_t = torch.clamp(gamma_t, min=1e-6)
        sorted_risks, _ = torch.sort(risks)
        n = sorted_risks.numel()
        if n == 0:
            raise ValueError("Risks tensor must contain at least one element.")
        u = (torch.arange(n, device=risks.device, dtype=risks.dtype) + 0.5) / n
        numerator = gamma_t * torch.exp(-gamma_t * (1.0 - u))
        denom = torch.clamp(1.0 - torch.exp(-gamma_t), min=1e-6)
        phi = numerator / denom
        return torch.dot(sorted_risks, phi) / n

    def __evar__(self, risks: torch.Tensor, alpha: float) -> torch.Tensor:
        """Entropic Value-at-Risk."""
        alpha_t = torch.as_tensor(alpha, dtype=risks.dtype, device=risks.device)
        if alpha_t.item() >= 1.0:
            raise ValueError("Alpha must be < 1 for EVaR.")
        flat = risks.view(-1)
        if flat.numel() == 0:
            raise ValueError("Risks tensor must contain at least one element.")
        tail = torch.clamp(1.0 - alpha_t, min=1e-6)
        log_tail = torch.log(tail)
        log_n = torch.log(torch.tensor(float(flat.numel()), device=flat.device, dtype=flat.dtype))
        t_vals = torch.logspace(-2, 1, steps=64, device=flat.device, dtype=flat.dtype)
