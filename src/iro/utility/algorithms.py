...
class EVAR(ERM):
    """
    Entropic Value-at-Risk minimization with learned alpha distribution.
    """

    def __init__(self, network, hparams, loss_fn):
        super(EVAR, self).__init__(network, hparams, loss_fn)
        self.entropic_dist = EntropicDistribution(self.loss_fn)
        self.aggregator = aggregation_function(name='evar')
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.grad_ratio = None

    def compute_evar_h(self, alpha, minibatches):
        risks = []
        for x, y in minibatches:
            t_alpha = torch.tile(alpha, (x.shape[0], 1))
            risks.append(self.loss_fn(self.network(x, t_alpha), y).reshape(1))
        risks = torch.cat(risks)
        return self.aggregator.aggregate(risks, alpha)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        a, b = self.entropic_dist.update(copy.deepcopy(self.network), minibatches)
        alphas = np.random.beta(a, b, size=10)
        alphas = torch.tensor(alphas, dtype=torch.float32).to(self.device)
        loss = torch.mean(torch.stack([self.compute_evar_h(alpha, minibatches) for alpha in alphas]))
...
class ESRM(ERM):
    """
    Exponential Spectral Risk Minimization (ESRM) using Arrow-Pratt coefficients.
    """
    def __init__(self, network, hparams, loss_fn):
        super(ESRM, self).__init__(network, hparams, loss_fn)
        self.arrow_pratt_dist = ArrowPrattDistribution(self.loss_fn)
        self.aggregator = aggregation_function(name='exponential')
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.grad_ratio = None
        self.num_gamma_samples = self.hparams.get("esrm_num_samples", 10)

    def compute_esrm_h(self, gamma, minibatches):
        gamma_tensor = torch.as_tensor(gamma, dtype=torch.float32, device=self.device)
        gamma_tensor = gamma_tensor.reshape(1, 1)
        risks = []
        for x, y in minibatches:
            t_gamma = gamma_tensor.repeat(x.shape[0], 1)
            risks.append(self.loss_fn(self.network(x, t_gamma), y).reshape(1))
        risks = torch.cat(risks)
        return self.aggregator.aggregate(risks, gamma_tensor.squeeze())
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        # Reset Adam if transitioning from ERM pretraining
        if self.update_count == self.hparams['erm_pretrain_iters']:
            lr_ = self.hparams["lr"]
            if self.hparams['erm_pretrain_iters'] > 0:
                lr_ /= self.hparams["lr_factor_reduction"]
        ...
        loc, scale = self.arrow_pratt_dist.update(copy.deepcopy(self.network), minibatches)
        gammas = np.random.lognormal(mean=loc, sigma=scale, size=self.num_gamma_samples)
        gammas = torch.tensor(gammas, dtype=torch.float32).to(self.device)
        loss = torch.mean(torch.stack([self.compute_esrm_h(gamma, minibatches) for gamma in gammas]))

        # Rescale gradients if using ERM pretraining
        if self.hparams['erm_pretrain_iters'] > 0:
            if self.grad_ratio is None:
                self.optimizer.zero_grad()
                loss.backward(retain_graph=True)
