# Adapted from https://github.com/facebookresearch/DomainBed/blob/main/domainbed/lib/kde.py
import math
import numpy as np
import torch
...
class Distribution1D:
    ...
    def create_dist(self):
        ...
        raise NotImplementedError("No distribution function was specified during initialization.")
...
class Nonparametric(Distribution1D):
    ...
    def estimate_parameters(self, x):
        ...
        if self.use_kde:
            self.bw = estimate_bandwidth(self.data, self.bw_select)
            self.kde = KernelDensityEstimator(self.data, bw_select=None, kernel="gaussian")

    def log_prob(self, x):
        if self.use_kde:
            return self.kde(x).log()
        else:
            _, counts = torch.unique(x, return_counts=True)
            return torch.log(counts / x.numel())

    def cdf(self, x):
        if self.use_kde:
            return self.kde.cdf(x)
        return torch.mean(self.data <= x)

    def icdf(self, q):
        """
        Quantile function (inverse CDF).
        """
        assert torch.all((q >= 0) & (q <= 1)), "Quantile must be between 0 and 1."
        if self.use_kde:
            cdf_f = lambda t: self.cdf(t)
            if len(q.shape) == 1:
                quantiles = torch.tensor([continuous_bisect_fun_left(cdf_f, elem, -2.0, 2.0) for elem in q])
            else:
                quantiles = continuous_bisect_fun_left(cdf_f, q, -2.0, 2.0)
            return quantiles
        else:
            if len(q.shape) == 1:
                quantiles = torch.stack([self.data[torch.ceil(elem * len(self.data) - 1).int()] for elem in q])
            else:
                quantiles = self.data[torch.ceil(q * len(self.data) - 1).int()]
            return quantiles
