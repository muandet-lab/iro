# Adapted from https://github.com/facebookresearch/DomainBed/blob/main/domainbed/lib/misc.py
import sys
import numpy as np
import torch
...
def cvar(network, loaders, loss_fn, device, alphas, not_h=True):
    ...
    for alpha in alphas:
        ...
        cvar_val = aggregator.aggregate(risks, alpha)
        acc_cvar = acc_for_cvar(risks, accs, alpha)
        cvars.append(round(cvar_val.item(), 2))
